...
docker_manifest_list_beta() {
  ...
  # docker manifest annotate ${TARGET}:beta ${TARGET}:${BUILD_VERSION}-slim-arm32v7 --os=linux --arch=arm --variant=v7
...
docker_manifest_list_testing() {
  ...
  # docker manifest annotate ${TARGET}:testing ${TARGET}:${BUILD_VERSION}-slim-arm32v7 --os=linux --arch=arm --variant=v7
